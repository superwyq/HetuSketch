...
行: [标题栏 38px] [内容区 1fr] [Sash 4px] [底部面板 动态] [状态栏 22px]
```
...
| --- | --- | --- | --- | --- |
| 标题栏 | 列 1/-1, 行 1 | `.workbench-titlebar` | `--titlebar-height` | 38px |
| 活动栏 | 列 1, 行 2/5 | `.activity-bar` | `--activity-width` | 48px |
| 主侧边栏 | 列 2, 行 2/5 | `.primary-sidebar` | `--primary-sidebar-width` | 250px（可折叠为 0） |
...
| 辅助侧边栏 | 列 6, 行 2/5 | `.secondary-sidebar` | `--secondary-sidebar-width` | 250px（可折叠为 0） |
| 水平 Sash | 列 1/-1, 行 3 | `.workbench-sash.sash-horizontal.panel-sash` | `--sash-size` | 4px |
| 底部面板 | 列 1/-1, 行 4 | `.bottom-panel` | `--panel-height` | 200px（可折叠为 0） |
...
通过 `appStore.themeMode` 控制（`light` / `dark`），持久化到 `hetusketch.workbench.theme.v1`。根容器添加 `theme-light` 或 `theme-dark` 类。

### 11.2 CSS 变量

工作台使用一组 CSS 变量定义颜色，深色为默认（`.workbench-shell`），浅色通过 `.workbench-shell.theme-light` 覆盖：

| 变量 | 深色值 | 浅色值 | 用途 |
| --- | --- | --- | --- |
| `--wb-bg` | `#1f1a14` | `#282018` | 工作台背景 |
| `--wb-activity` | `#24170f` | `#2d1b10` | 活动栏背景 |
| `--wb-titlebar` | `#2d1b10` | `#341f12` | 标题栏背景 |
| `--wb-titlebar-text` | `#fff3de` | `#fff6e8` | 标题栏文字 |
| `--wb-titlebar-muted` | `#f0c58d` | `#f3c98f` | 标题栏次要文字 |
| `--wb-sidebar` | `#241d16` | `#f4eadb` | 主侧边栏背景 |
| `--wb-sidebar-2` | `#211a14` | `#fff7ea` | 辅助侧边栏背景 |
| `--wb-editor` | `#f4eadb` | `#efe0c7` | 编辑器背景 |
| `--wb-editor-2` | `#fffaf1` | `#fffaf1` | 编辑器次背景 |
| `--wb-panel` | `#231c15` | `#fff5e8` | 底部面板背景 |
| `--wb-border` | `rgba(255,232,196,0.22)` | `rgba(78,49,26,0.18)` | 边框 |
| `--wb-text` | `#f8ead2` | `#2b2118` | 主文字 |
| `--wb-muted` | `#d7b98f` | `#6b4f35` | 次要文字 |
| `--wb-accent` | `#f0a24a` | `#f0a24a` | 强调色（橙） |
| `--wb-blue` | `#4da3ff` | `#4da3ff` | 蓝色辅助 |

### 11.3 字体变量

字体通过 CSS 变量注入，由 `appStore` 的 `sidebarFont` 和 `editorFont` 控制：
...
所有样式集中在 `src/renderer/src/styles.css`，按以下顺序组织：

1. **基础重置**：`:root`、`body`、滚动条样式
2. **主题变量**：`.theme-dark`、`.theme-light`
3. **应用骨架**：`.app-shell`、`.app-sider`、`.app-header`、`.app-content`（旧版布局，部分页面仍使用）
4. **通用组件**：`.page-stack`、`.feature-card`、`.metric-card`、`.hero-panel` 等
5. **页面样式**：Dashboard、Entries、WritingStudio、Checks、Settings、QuickLookup 等
6. **关系图谱**：`.graph-workspace`、`.graph-canvas`、`.graph-filter-panel` 等
7. **工作台外壳**（约 1172 行起）：`.workbench-shell`、`.workbench-titlebar`、`.activity-bar`、`.primary-sidebar`、`.secondary-sidebar`、`.editor-area`、`.editor-group`、`.editor-tabbar`、`.bottom-panel`、`.status-bar`、`.workbench-sash` 等
